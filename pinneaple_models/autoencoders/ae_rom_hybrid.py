from __future__ import annotations

from typing import Dict, List, Optional

import torch
import torch.nn as nn

from .base import AEBase, AEOutput
from .dense_ae import _mlp


class AEROMHybrid(AEBase):
    """
    AE-ROM Hybrid (MVP):
      - Dense AE
      - Linear latent ROM: z_next ≈ A z + b
      - Optional "dxdt" training: z_dot ≈ A z (if user provides dz target later)

    forward supports:
      - forward(x)
      - forward(x, x_next=...) adds rom loss

    Args:
      input_dim, latent_dim, hidden
      rom_weight: weight for rom consistency loss
    """
    def __init__(
        self,
        input_dim: int,
        latent_dim: int,
        hidden: List[int] = (512, 256),
        activation: str = "gelu",
        rom_weight: float = 1.0,
    ):
        super().__init__()
        act = {"tanh": nn.Tanh(), "relu": nn.ReLU(), "gelu": nn.GELU()}.get(activation.lower(), nn.GELU())
        self.rom_weight = float(rom_weight)

        self.encoder = _mlp([input_dim, *list(hidden), latent_dim], act, last_act=False)
        self.decoder = _mlp([latent_dim, *list(reversed(hidden)), input_dim], act, last_act=False)

        self.A = nn.Parameter(torch.eye(latent_dim))
        self.b = nn.Parameter(torch.zeros(latent_dim))

    def encode(self, x: torch.Tensor) -> torch.Tensor:
        x = x.view(x.shape[0], -1)
        return self.encoder(x)

    def decode(self, z: torch.Tensor) -> torch.Tensor:
        return self.decoder(z)

    def forward(self, x: torch.Tensor, *, x_next: Optional[torch.Tensor] = None) -> AEOutput:
        z = self.encode(x)
        x_hat = self.decode(z)
        losses = self.loss_from_parts(x_hat=x_hat, z=z, x=x)

        extras: Dict[str, torch.Tensor] = {"A": self.A, "b": self.b}

        if x_next is not None:
            z_next = self.encode(x_next)
            z_pred = z @ self.A.t() + self.b[None, :]
            rom = torch.mean((z_next - z_pred) ** 2)
            losses["rom"] = rom
            losses["total"] = losses["total"] + self.rom_weight * rom
            extras["z_next"] = z_next
            extras["z_pred"] = z_pred

        return AEOutput(x_hat=x_hat, z=z, losses=losses, extras=extras)
